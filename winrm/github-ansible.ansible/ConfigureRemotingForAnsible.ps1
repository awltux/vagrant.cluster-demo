...
{
    $thumbprint = (Get-ChildItem -Path cert:\LocalMachine\My | ?{ $_.Subject -Like "*CN=${SubjectName}*"  }).thumbprint
    Write-HostLog "SSL certificate found; thumbprint: $thumbprint"
    ...
    Write-Verbose "valueset = $valueset"

    $selectorset = @{
        Address = "*"
        Transport = "HTTPS"
    }
    Write-Verbose "selectorset = $selectorset"
